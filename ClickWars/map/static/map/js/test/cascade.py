...
# Code follows:
var.registers(['r2d', 'getTiles', 'jsonToTiles', 'mergeTiles', 'polygonCover', 'appendHashTiles', 'pointToTile', 'toID', 'fromID', 'tilesForEveryZoom', 'lineCover', 'compareTiles', 'pointToTileFraction', 'd2r'])
@Js
def PyJsHoisted_getTiles_(geom, limits, this, arguments, var=var):
    var = Scope({'arguments':arguments, 'this':this, 'geom':geom, 'limits':limits}, var)
    var.registers(['coords', 'i', 'geom', 'tileHash', 'maxZoom', 'tiles', 'tile', 't', 'len', 'limits'])
...
def PyJsHoisted_tilesForEveryZoom_(polygon, this, arguments, var=var):
    ...
    var.registers(['limitszoom14', 'limitszoom12', 'polygon', 'limitszoom16'])
    PyJs_Object_2_ = Js({'min_zoom':Js(12.0),'max_zoom':Js(12.0)})
    var.put('limitszoom12', PyJs_Object_2_)
    PyJs_Object_3_ = Js({'min_zoom':Js(14.0),'max_zoom':Js(14.0)})
    var.put('limitszoom14', PyJs_Object_3_)
    PyJs_Object_4_ = Js({'min_zoom':Js(16.0),'max_zoom':Js(16.0)})
    var.put('limitszoom16', PyJs_Object_4_)
    var.get('jsonToTiles')(var.get('polygon'), var.get('limitszoom12'))
    var.get('jsonToTiles')(var.get('polygon'), var.get('limitszoom14'))
    var.get('jsonToTiles')(var.get('polygon'), var.get('limitszoom16'))
...
def PyJsHoisted_mergeTiles_(tileHash, tiles, limits, this, arguments, var=var):
    ...
    var.registers(['id2', 'mergedTiles', 'i', 'parentTile', 'parentTiles', 'tileHash', 'parentTileHash', 'z', 'id3', 'tiles', 'id4', 't', 'limits'])
...
def PyJsHoisted_polygonCover_(tileHash, tileArray, geom, zoom, this, arguments, var=var):
    var = Scope({'geom':geom, 'tileHash':tileHash, 'arguments':arguments, 'zoom':zoom, 'tileArray':tileArray, 'this':this}, var)
    var.registers(['k', 'x', 'i', 'y', 'geom', 'id', 'm', 'tileHash', 'intersections', 'zoom', 'tileArray', 'j', 'ring', 'len'])
...
def PyJsHoisted_appendHashTiles_(hash, tiles, this, arguments, var=var):
    var = Scope({'hash':hash, 'this':this, 'tiles':tiles, 'arguments':arguments}, var)
    var.registers(['keys', 'i', 'hash', 'tiles'])
...
def PyJsHoisted_pointToTile_(lon, lat, z, this, arguments, var=var):
    var = Scope({'lat':lat, 'lon':lon, 'z':z, 'this':this, 'arguments':arguments}, var)
...
def PyJsHoisted_toID_(x, y, z, this, arguments, var=var):
    var = Scope({'x':x, 'arguments':arguments, 'z':z, 'y':y, 'this':this}, var)
    var.registers(['x', 'dim', 'z', 'y'])
...
def PyJsHoisted_lineCover_(tileHash, coords, maxZoom, ring, this, arguments, var=var):
    var = Scope({'coords':coords, 'ring':ring, 'tileHash':tileHash, 'maxZoom':maxZoom, 'this':this, 'arguments':arguments}, var)
    var.registers(['x', 'sx', 'tileHash', 'tMaxY', 'sy', 'prevX', 'dy', 'maxZoom', 'start', 'ring', 'coords', 'i', 'tdx', 'stop', 'x1', 'tdy', 'tMaxX', 'y0', 'prevY', 'y1', 'y', 'x0', 'dx'])
    pass
    #for JS loop
    var.put('i', Js(0.0))
...
def PyJsHoisted_jsonToTiles_(polygonjson, limits, this, arguments, var=var):
    var = Scope({'polygonjson':polygonjson, 'arguments':arguments, 'this':this, 'limits':limits}, var)
    var.registers(['x', 'i', 'y', 'polygonjson', 'tilesRaw', 'z', 'limits'])
    ...
    while (var.get('i')<var.get('tilesRaw').get('length')):
        try:
            ...
            var.get('console').callprop('log', var.get('tileUrl'))
        finally:
                (var.put('i',Js(var.get('i').to_number())+Js(1))-Js(1))
...
def PyJsHoisted_pointToTileFraction_(lon, lat, z, this, arguments, var=var):
    var = Scope({'lat':lat, 'lon':lon, 'z':z, 'this':this, 'arguments':arguments}, var)
    var.registers(['x', 'y', 'lon', 'z2', 'sin', 'z', 'lat'])
